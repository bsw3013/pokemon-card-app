...
// 3. Remove rawSaveTimeoutRef and rawDbSaving
admin = admin.replace(
...
// 5. Fix err in handleRawDeleteRow
admin = admin.replace(
`      } catch(err) {
          alert('?? ?');
      }`,
...
// 6. Fix immutability in handleRawAddColumn
admin = admin.replace(
`      try {
...
// 7. Fix immutability in handleRawDeleteColumn
admin = admin.replace(
`      try {
           const BATCH_LIMIT = 400;
           let batch = writeBatch(db);
...
// 2. Extract handlePickerSearch to before useEffect
